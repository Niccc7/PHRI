...
@section('content')
    <div class="card mb-0">
        <div class="card-body">
            <h4 class="card-title text-center">Data Jenis Usaha</h4>
                    ...
                    <tbody>
                        @foreach ($jenis as $j)
                            <tr>
                                ...
                                <th>{{ $j->name }}</th>
                                <th>
                                    <button type="button" class="btn btn-sm btn-warning edit-jenis border-0"
                                        value="{{ $j->id }}">
                                        Edit
                                    </button>
                                    <form action="{{ route('dashboard.jenis.destroy', ['id' => $j->id]) }}" method="post"
                                        class="d-inline">
                                        @method('delete')
                                        @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content bg-white">
                <div class="modal-header">
                    <h1 class="modal-title fs-5 text-black" id="exampleModalLabel">Form Tambah Jenis Usaha</h1>
                    ...
                    <form id="form_store-jenis" class="mb-5">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                id="name" name="name" required autofocus placeholder="misal: Pribadi">
                        </div>
            ...
            <div class="modal-content bg-white">
                ...
                <div class="modal-body">
                    <form method="post" id="form_update" class="mb-5">
                        @csrf
                        <input type="hidden" id="edit_jenis_id">
                        <div class="mb-3">
                            <label for="name" class="form-label">Name</label><span style="margin-left: 1px;color: red">*</span>
                            <input type="text" class="form-control"
...
@section('script')
    <script>
        ...
        $(function() {
            $(document).on('submit', '#form_store-jenis', function(e) {
                e.preventDefault();
                $.ajax({
                    type: "POST",
                    url: "{{route('dashboard.jenis.store')}}",
            ...
            $(document).on('click', '.edit-jenis', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('dashboard.jenis.edit', ':id') }}".replace(':id', jenis_id),
                    success: function(response) {
                        if (response.status === 200) {
                            $('#edit_name').val(response.jenis.name);
                        } else {
            ...
            $(document).on('submit', '#form_update', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('dashboard.jenis.update', ':id') }}".replace(':id', jenis_id),
                    data: new FormData(this),
                    cache: false,
                    contentType: false,
                    processData: false,
